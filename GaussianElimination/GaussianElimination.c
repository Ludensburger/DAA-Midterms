...
int main() {
    ...
    float matrix[ROWS][COLS] = {{2, -1, 1, 1},
                                {4, 1, -1, 5},
                                {1, 1, 1, 0}};
    ...
    for (int i = 0; i < COLS; ++i) {
        ...
        // Handles division by zero, if pivot element is zero
        if (matrix[0][i] == 0) {
            printf("Mathematical error: Division by zero\n");
            return -1;
        }
        newRow1[i] = (-2 * matrix[0][i]) + matrix[1][i];     // Correct formula: -2R1 + R2
        newRow2[i] = ((-0.5) * matrix[0][i]) + matrix[2][i]; // Correct formula: -1/2R1 + R3
    }
    ...
    for (int i = 0; i < COLS; ++i) {
        matrix[2][i] = ((-0.5) * matrix[1][i]) + matrix[2][i];
    }
